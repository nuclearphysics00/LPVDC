...
// DrawElectrodes: スタック確保 + DrawClone() でメモリリーク修正済み
// (旧: new TLine/TEllipse → Draw() はポインタを失いパッド依存の暗黙オーナーシップに頼っていた)
...
int main(int argc, char** argv) {
  ...
  //
  // 例:
  //   ./track_reco_avalanche_pap_wirecath 1
  //   ./track_reco_avalanche_pap_wirecath 1 10
  //   ./track_reco_avalanche_pap_wirecath 1 root/.../t_hist_nt.csv 10
  //   EVENTS_PER_JOB=10 ./track_reco_avalanche_pap_wirecath 1
  int job_seed = 1;
  int eventsPerJob = 1;
  ...
  if (const char* envEvents = std::getenv("EVENTS_PER_JOB")) {
    ...
    const char* defaultTimeHistFile = "root/run_20260322_202445_pap_wire/analysis_L0_prim/t_hist_nt.csv";
  #elif defined(GEOM_PLATE)
    const char* geomTag = "plate";
    const char* defaultTimeHistFile = "root/run_20260322_202450_pap_plate/analysis_L0_prim/t_hist_nt.csv";
  #else
