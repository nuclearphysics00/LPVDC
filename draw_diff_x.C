...
void draw_diff_x(const char* input_root = "merged_track_results.root",
  ...
  if (!tree) {
    std::cerr << "[ERROR] tree 'tree' not found in: " << input_root << std::endl;
    f->Close();
    return;
  }
  ...
  TH1D* h = new TH1D("h_diff_x", "Position residual;diff_x = x_{reco} - x_{true} [cm];Entries",
                     nbins, xmin, xmax);
  ...
  if (!fout) {
    std::cerr << "[WARN] cannot open csv for writing: " << output_csv << std::endl;
  } else {
