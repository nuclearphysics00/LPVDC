...
int main(int argc, char** argv) {
  ...
  const char* timeHistFile =
      (argc > 1) ? argv[1] : "./root/run_20260322_205404/analysis_L0_prim_exact/t_hist_nt.csv";
  ...
  fs::path outBaseDir = fs::path("opt_20260426_ap");
...
#if defined(GEOM_PLATE)
    geo = Detector::PAP_PlaneCathode_Periodic(0.40, 0.50, 0.0, rAnode, rPW, vAnode, vPW, vCat);
#elif defined(GEOM_WIRE)
    geo = Detector::PAP_WireCathode_Periodic(0.40, 0.50, 0.0, 0.20, 0, rAnode, rPW, rCat,
                                             vAnode, vPW, vCat);
#endif
