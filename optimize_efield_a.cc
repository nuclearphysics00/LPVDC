...
#include "detector_geometry.hh"
  ...
  #include "geometry/geom_a_plate.hh"
#elif defined(GEOM_WIRE)
  #include "geometry/geom_a_wirecath.hh"
#else
...
struct ExProfileForOverlay {
  ...
  double vPW = 0.0;
  double vCat = 0.0;
  double pitchA_cm = 0.0;
  double pitchA_mm = 0.0;
  std::vector<double> y;
...
struct ConfigRankingEntry {
  ...
  // A-only Vcath x anode-pitch scan score components.
  // Raw ex2_mean is kept unnormalized for direct comparison between configurations.
  double sAOnlyNorm = -1.0;
  double sLtRmsNorm = -1.0;
  double sDxNorm = -1.0;
  double sCollectLossNorm = -1.0;
  int nAtrans = 0;
  int nAalign = 0;
  int nPWmirror = 0;
  int nPWalign = 0;
...
// ==================== A-only Vcath x anode-pitch scoring / plots ====================
static void ComputeAOnlyScores(std::vector<ConfigRankingEntry>& entries) {
  std::vector<double> ex2Vals;
  std::vector<double> rmsVals;
  std::vector<double> dxVals;
  std::vector<double> lossVals;

  for (const auto& e : entries) {
    if (std::isfinite(e.ex2_mean) && e.ex2_mean >= 0.0) ex2Vals.push_back(e.ex2_mean);
    if (e.n_points > 0 && std::isfinite(e.residual_rms) && e.residual_rms >= 0.0) {
      rmsVals.push_back(e.residual_rms);
    }
    if (std::isfinite(e.mean_dx_end) && e.mean_dx_end >= 0.0) dxVals.push_back(e.mean_dx_end);
    if (std::isfinite(e.collect_eff)) {
      const double loss = std::max(0.0, 1.0 - e.collect_eff);
      if (loss > 0.0) lossVals.push_back(loss);
    }
  }

  const double eps = 1.0e-30;
  const double medEx2  = std::max(Median(ex2Vals), eps);
  const double medRms  = std::max(Median(rmsVals), eps);
  const double medDx   = std::max(Median(dxVals),  eps);
  const bool useLoss = !lossVals.empty();
  const double medLoss = useLoss ? std::max(Median(lossVals), eps) : 1.0;

  const double wEx2  = 1.0;
  const double wRms  = 1.0;
  const double wDx   = 1.0;
  const double wLoss = 1.0;

  for (auto& e : entries) {
    if (!std::isfinite(e.ex2_mean) || e.ex2_mean < 0.0) {
      e.sEx2Norm = -1.0;
      e.sLtRmsNorm = -1.0;
      e.sDxNorm = -1.0;
      e.sCollectLossNorm = -1.0;
      e.sAOnlyNorm = -1.0;
      continue;
    }

    e.sEx2Norm = e.ex2_mean / medEx2;
    e.sLtRmsNorm = (e.n_points > 0 && std::isfinite(e.residual_rms) && e.residual_rms >= 0.0)
                     ? e.residual_rms / medRms : 0.0;
    e.sDxNorm = (std::isfinite(e.mean_dx_end) && e.mean_dx_end >= 0.0)
                  ? e.mean_dx_end / medDx : 0.0;
    if (useLoss && std::isfinite(e.collect_eff)) {
      e.sCollectLossNorm = std::max(0.0, 1.0 - e.collect_eff) / medLoss;
    } else {
      e.sCollectLossNorm = 0.0;
    }

    e.sAOnlyNorm = wEx2  * e.sEx2Norm
                 + wRms  * e.sLtRmsNorm
                 + wDx   * e.sDxNorm
                 + wLoss * e.sCollectLossNorm;
  }
}

static void WriteAOnlyVcatPitchScoreCsv(const fs::path& outPath,
                                        const std::vector<ConfigRankingEntry>& entries) {
  std::ofstream fout(outPath);
  if (!fout) {
    std::cerr << "[warn] Failed to write A-only score csv: " << outPath << "\n";
    return;
  }

  fout << "config_name,Vcath_V,pitchA_mm,pitchA_cm,"
          "Ex2_mean,Ex_abs_max,Ex_mean_abs,"
          "Lt_RMS_cm,Lt_R2,collect_eff,mean_dx_end,"
          "S_Ex2_norm,S_LtRMS_norm,S_dx_norm,S_collectLoss_norm,S_Aonly_norm\n";
...
static void WriteAOnlyVcatPitchRankingMarkdown(const fs::path& outPath,
                                               const std::vector<ConfigRankingEntry>& entries,
                                               const int topN = 20) {
  std::ofstream fout(outPath);
  if (!fout) {
    std::cerr << "[warn] Failed to write A-only ranking markdown: " << outPath << "\n";
    return;
  }

  fout << "# A-only Vcath x anode-pitch ranking\n\n";
  fout << "- Main scan variables: cathode voltage Vcath and A-A pitch.\n";
  fout << "- Raw `<Ex^2>` is not normalized and can be compared directly between configurations.\n";
  fout << "- `S_Aonly` is only a normalized combined diagnostic score.\n";
  fout << "- Smaller is better for `<Ex^2>`, Lt RMS, mean |dx_end|, and S_Aonly.\n";
  fout << "- Larger is better for R^2 and collect efficiency.\n\n";
  fout << "Total configurations: " << entries.size() << "\n\n";

  auto write_header = [&](const std::string& title, bool score) {
    fout << "## " << title << "\n\n";
    if (score) {
      fout << "| Rank | Vcath [V] | A-A pitch [mm] | S_Aonly | <Ex^2> [(V/cm)^2] | max|Ex| [V/cm] | Lt RMS [cm] | R^2 | collectEff | mean|dx_end| [cm] | Config |\n";
      fout << "|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---|\n";
    } else {
      fout << "| Rank | Vcath [V] | A-A pitch [mm] | <Ex^2> [(V/cm)^2] | max|Ex| [V/cm] | Lt RMS [cm] | R^2 | collectEff | mean|dx_end| [cm] | Config |\n";
      fout << "|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---|\n";
    }
  };

  auto write_row = [&](int rank, const ConfigRankingEntry& e, bool score) {
    fout << "| " << rank
         << " | " << e.vCat
         << " | " << e.pitchA_mm;
    if (score) fout << " | " << e.sAOnlyNorm;
    fout << " | " << e.ex2_mean
         << " | " << e.ex_abs_max
         << " | " << e.residual_rms
         << " | " << e.r2
  ...
  {
    auto v = entries;
    v.erase(std::remove_if(v.begin(), v.end(), [](const auto& e) {
      return !std::isfinite(e.ex2_mean) || e.ex2_mean < 0.0;
    }), v.end());
    std::sort(v.begin(), v.end(), [](const auto& a, const auto& b) { return a.ex2_mean < b.ex2_mean; });
    write_header("Top smallest raw <Ex^2>", false);
    for (int i = 0; i < std::min<int>(topN, v.size()); ++i) write_row(i + 1, v[i], false);
    fout << "\n";
  }

  {
    auto v = entries;
    v.erase(std::remove_if(v.begin(), v.end(), [](const auto& e) {
      return !std::isfinite(e.sAOnlyNorm) || e.sAOnlyNorm < 0.0;
    }), v.end());
    std::sort(v.begin(), v.end(), [](const auto& a, const auto& b) { return a.sAOnlyNorm < b.sAOnlyNorm; });
    write_header("Top smallest A-only total score", true);
    for (int i = 0; i < std::min<int>(topN, v.size()); ++i) write_row(i + 1, v[i], true);
    fout << "\n";
  }

  {
    auto v = entries;
    v.erase(std::remove_if(v.begin(), v.end(), [](const auto& e) {
      return e.n_points <= 0 || !std::isfinite(e.residual_rms) || e.residual_rms < 0.0;
    }), v.end());
    std::sort(v.begin(), v.end(), [](const auto& a, const auto& b) { return a.residual_rms < b.residual_rms; });
    write_header("Top smallest L-t residual RMS", false);
    for (int i = 0; i < std::min<int>(topN, v.size()); ++i) write_row(i + 1, v[i], false);
    fout << "\n";
  }

  {
    auto v = entries;
    v.erase(std::remove_if(v.begin(), v.end(), [](const auto& e) {
      return e.n_points <= 0 || !std::isfinite(e.r2);
    }), v.end());
    std::sort(v.begin(), v.end(), [](const auto& a, const auto& b) { return a.r2 > b.r2; });
    write_header("Top largest L-t R^2", false);
    for (int i = 0; i < std::min<int>(topN, v.size()); ++i) write_row(i + 1, v[i], false);
    fout << "\n";
  }
}

static std::string SafeFloatTag(double x, int ndigit = 2) {
  char buf[64];
  std::snprintf(buf, sizeof(buf), "%.*f", ndigit, x);
  std::string s(buf);
  for (char& c : s) {
    if (c == '-') c = 'm';
    else if (c == '+') c = 'p';
...
static void SaveEx2VsVcatForFixedPitchPlot(const fs::path& outDir,
                                           const std::vector<ConfigRankingEntry>& entries,
                                           const double fixedPitchMm,
                                           const double pitchToleranceMm = 1.0e-6) {
  fs::create_directories(outDir);

  std::vector<ConfigRankingEntry> selected;
  for (const auto& e : entries) {
    if (e.ex2_mean < 0.0 || !std::isfinite(e.ex2_mean)) continue;
    if (std::abs(e.pitchA_mm - fixedPitchMm) <= pitchToleranceMm) selected.push_back(e);
  }
  if (selected.empty()) {
    std::cerr << "[warn] No entries found for fixed pitch " << fixedPitchMm << " mm\n";
    return;
  }
  std::sort(selected.begin(), selected.end(), [](const auto& a, const auto& b) { return a.vCat < b.vCat; });

  std::vector<double> xVcat, yEx2;
  for (const auto& e : selected) {
    xVcat.push_back(e.vCat);
    yEx2.push_back(e.ex2_mean);
  }

  auto c = std::make_unique<TCanvas>(Form("cEx2VsVcat_pitch%.2fmm", fixedPitchMm),
                                     Form("Ex2 vs Vcath, pitch %.2f mm", fixedPitchMm), 900, 700);
  c->SetGrid();

  auto g = std::make_unique<TGraph>((int)xVcat.size(), xVcat.data(), yEx2.data());
  g->SetName(Form("gEx2_vs_Vcath_pitch_%.2fmm", fixedPitchMm));
  g->SetTitle(Form("<E_{x}^{2}> vs V_{cath} at A-A pitch = %.2f mm;Cathode voltage V_{cath} [V];<E_{x}^{2}> [(V/cm)^{2}]", fixedPitchMm));
  g->SetMarkerStyle(20);
  g->SetMarkerSize(1.0);
  g->SetLineWidth(2);
  g->Draw("APL");

  auto bestIt = std::min_element(selected.begin(), selected.end(), [](const auto& a, const auto& b) { return a.ex2_mean < b.ex2_mean; });
  std::unique_ptr<TGraph> gBest;
  if (bestIt != selected.end()) {
    const double xBest = bestIt->vCat;
    const double yBest = bestIt->ex2_mean;
    gBest = std::make_unique<TGraph>(1, &xBest, &yBest);
    gBest->SetName("gBestEx2Point");
    gBest->SetMarkerStyle(29);
    gBest->SetMarkerSize(2.0);
    gBest->SetMarkerColor(kRed + 1);
    gBest->Draw("P SAME");
    ...
    latex.SetTextSize(0.032);
    latex.SetTextAlign(13);
    latex.DrawLatex(0.15, 0.88, Form("A-A pitch = %.2f mm", fixedPitchMm));
    latex.DrawLatex(0.15, 0.84, Form("Best: V_{cath} = %.0f V", bestIt->vCat));
    latex.DrawLatex(0.15, 0.80, Form("<E_{x}^{2}> = %.6e", bestIt->ex2_mean));
    latex.DrawLatex(0.15, 0.76, Form("max|E_{x}| = %.6e", bestIt->ex_abs_max));
  }

  const std::string safeTag = "pitch_" + SafeFloatTag(fixedPitchMm, 2) + "mm";
  const fs::path pngPath  = outDir / ("Ex2_vs_Vcath_" + safeTag + ".png");
  const fs::path pdfPath  = outDir / ("Ex2_vs_Vcath_" + safeTag + ".pdf");
  const fs::path rootPath = outDir / ("Ex2_vs_Vcath_" + safeTag + ".root");
  const fs::path csvPath  = outDir / ("Ex2_vs_Vcath_" + safeTag + ".csv");

  c->SaveAs(pngPath.string().c_str());
  c->SaveAs(pdfPath.string().c_str());

  {
    TFile fout(rootPath.string().c_str(), "RECREATE");
    g->Write();
    if (gBest) gBest->Write();
    TTree t("scan", "Ex2 vs Vcath at fixed A-A pitch");
    double pitchA_mm = fixedPitchMm, pitchA_cm = fixedPitchMm / 10.0;
    double vCath_V = 0.0, ex2_mean = 0.0, ex_abs_max = 0.0, ex_mean_abs = 0.0, rms_cm = 0.0, r2 = 0.0;
    char config_name[256];
    t.Branch("pitchA_mm", &pitchA_mm);
    t.Branch("pitchA_cm", &pitchA_cm);
    t.Branch("Vcath_V", &vCath_V);
    t.Branch("Ex2_mean", &ex2_mean);
    t.Branch("Ex_abs_max", &ex_abs_max);
    t.Branch("Ex_mean_abs", &ex_mean_abs);
    t.Branch("Lt_RMS_cm", &rms_cm);
    t.Branch("Lt_R2", &r2);
    t.Branch("config_name", config_name, "config_name/C");
    for (const auto& e : selected) {
      pitchA_mm = e.pitchA_mm;
      pitchA_cm = e.pitchA_cm;
      vCath_V = e.vCat;
      ex2_mean = e.ex2_mean;
      ex_abs_max = e.ex_abs_max;
      ex_mean_abs = e.ex_mean_abs;
      rms_cm = e.residual_rms;
      r2 = e.r2;
      std::snprintf(config_name, sizeof(config_name), "%s", e.conf_name.c_str());
      t.Fill();
    }
    t.Write();
    c->Write("cEx2_vs_Vcath_fixedPitch");
    fout.Close();
  }

  std::ofstream fout(csvPath);
  if (fout) {
    fout << "pitchA_mm,Vcath_V,Ex2_mean,Ex_abs_max,Ex_mean_abs,Lt_RMS_cm,Lt_R2,config_name\n";
    for (const auto& e : selected) {
      fout << e.pitchA_mm << "," << e.vCat << "," << e.ex2_mean << "," << e.ex_abs_max << ","
           << e.ex_mean_abs << "," << e.residual_rms << "," << e.r2 << "," << e.conf_name << "\n";
    }
  }
}

static void SaveEx2VsPitchForFixedVcatPlot(const fs::path& outDir,
                                           const std::vector<ConfigRankingEntry>& entries,
                                           const double fixedVcat,
                                           const double vcatTolerance = 1.0e-6) {
  fs::create_directories(outDir);

  std::vector<ConfigRankingEntry> selected;
  for (const auto& e : entries) {
    if (e.ex2_mean < 0.0 || !std::isfinite(e.ex2_mean)) continue;
    if (std::abs(e.vCat - fixedVcat) <= vcatTolerance) selected.push_back(e);
  }
  if (selected.empty()) {
    std::cerr << "[warn] No entries found for fixed Vcath " << fixedVcat << " V\n";
    return;
  }
  std::sort(selected.begin(), selected.end(), [](const auto& a, const auto& b) { return a.pitchA_mm < b.pitchA_mm; });

  std::vector<double> xPitch, yEx2;
  for (const auto& e : selected) {
    xPitch.push_back(e.pitchA_mm);
    yEx2.push_back(e.ex2_mean);
  }

  auto c = std::make_unique<TCanvas>(Form("cEx2VsPitch_Vcath%.0fV", fixedVcat),
                                     Form("Ex2 vs pitch, Vcath %.0f V", fixedVcat), 900, 700);
  c->SetGrid();

  auto g = std::make_unique<TGraph>((int)xPitch.size(), xPitch.data(), yEx2.data());
  g->SetName(Form("gEx2_vs_pitch_Vcath_%.0fV", fixedVcat));
  g->SetTitle(Form("<E_{x}^{2}> vs A-A pitch at V_{cath} = %.0f V;A-A pitch [mm];<E_{x}^{2}> [(V/cm)^{2}]", fixedVcat));
  g->SetMarkerStyle(20);
  g->SetMarkerSize(1.0);
  g->SetLineWidth(2);
  g->Draw("APL");
  ...
  std::unique_ptr<TGraph> gBest;
  if (bestIt != selected.end()) {
    const double xBest = bestIt->pitchA_mm;
    const double yBest = bestIt->ex2_mean;
    gBest = std::make_unique<TGraph>(1, &xBest, &yBest);
    gBest->SetName("gBestEx2Point");
    gBest->SetMarkerStyle(29);
    ...
    latex.SetTextSize(0.032);
    latex.SetTextAlign(13);
    latex.DrawLatex(0.15, 0.88, Form("V_{cath} = %.0f V", fixedVcat));
    latex.DrawLatex(0.15, 0.84, Form("Best: A-A pitch = %.2f mm", bestIt->pitchA_mm));
    latex.DrawLatex(0.15, 0.80, Form("<E_{x}^{2}> = %.6e", bestIt->ex2_mean));
    latex.DrawLatex(0.15, 0.76, Form("max|E_{x}| = %.6e", bestIt->ex_abs_max));
  }

  const std::string safeTag = "Vcath_" + SafeFloatTag(fixedVcat, 0) + "V";
  const fs::path pngPath  = outDir / ("Ex2_vs_pitch_" + safeTag + ".png");
  const fs::path pdfPath  = outDir / ("Ex2_vs_pitch_" + safeTag + ".pdf");
  const fs::path rootPath = outDir / ("Ex2_vs_pitch_" + safeTag + ".root");
  const fs::path csvPath  = outDir / ("Ex2_vs_pitch_" + safeTag + ".csv");
...

  {
    ...
    g->Write();
    if (gBest) gBest->Write();
    TTree t("scan", "Ex2 vs A-A pitch at fixed Vcath");
    double vCath_V = fixedVcat;
    double pitchA_mm = 0.0, pitchA_cm = 0.0, ex2_mean = 0.0, ex_abs_max = 0.0, ex_mean_abs = 0.0, rms_cm = 0.0, r2 = 0.0;
    char config_name[256];
    t.Branch("Vcath_V", &vCath_V);
    t.Branch("pitchA_mm", &pitchA_mm);
    t.Branch("pitchA_cm", &pitchA_cm);
    ...
    t.Write();
    c->Write("cEx2_vs_pitch_fixedVcath");
    fout.Close();
  }

  std::ofstream fout(csvPath);
  if (fout) {
    fout << "Vcath_V,pitchA_mm,pitchA_cm,Ex2_mean,Ex_abs_max,Ex_mean_abs,Lt_RMS_cm,Lt_R2,config_name\n";
    for (const auto& e : selected) {
      fout << e.vCat << "," << e.pitchA_mm << "," << e.pitchA_cm << "," << e.ex2_mean << ","
           << e.ex_abs_max << "," << e.ex_mean_abs << "," << e.residual_rms << "," << e.r2 << "," << e.conf_name << "\n";
    }
  }
}

// Helpers for regular-grid heat maps.
// TGraph2D is useful for a 3D surface, but for a Vcath x pitch grid
// TH2D gives stable axis ranges and avoids the single-point auto-range issue.
static std::vector<double> UniqueSortedWithTol(std::vector<double> v,
...
static int FindIndexWithTol(const std::vector<double>& v,
                            const double x,
                            const double tol = 1.0e-6) {
  for (size_t i = 0; i < v.size(); ++i) {
    if (std::abs(v[i] - x) <= tol) {
...
static std::vector<double> MakeBinEdgesFromCenters(const std::vector<double>& centers) {
  std::vector<double> edges;

  if (centers.empty()) return edges;

  if (centers.size() == 1) {
    const double w = std::max(0.05, 0.1 * std::abs(centers.front()));
    edges.push_back(centers.front() - w);
    edges.push_back(centers.front() + w);
    return edges;
  }
  ...
  edges[centers.size()] = centers.back() +
                           0.5 * (centers.back() - centers[centers.size() - 2]);

  return edges;
}

static void SaveEx2PitchVcatHeatmapTH2(
    const fs::path& outDir,
    const std::vector<ConfigRankingEntry>& entries) {
  fs::create_directories(outDir);

  std::vector<ConfigRankingEntry> valid;
  std::vector<double> pitchVals;
  std::vector<double> vcatVals;

  for (const auto& e : entries) {
    if (!std::isfinite(e.ex2_mean) || e.ex2_mean < 0.0) continue;
    if (!std::isfinite(e.pitchA_mm)) continue;
    if (!std::isfinite(e.vCat)) continue;

    valid.push_back(e);
    pitchVals.push_back(e.pitchA_mm);
    vcatVals.push_back(e.vCat);
  }

  if (valid.empty()) {
    std::cerr << "[warn] No valid entries for TH2 Ex2 heatmap.\n";
    return;
  }

  const auto pitches = UniqueSortedWithTol(pitchVals, 1.0e-5);
  const auto vcats   = UniqueSortedWithTol(vcatVals,   1.0e-5);

  std::cerr << "[TH2 Ex2 map] valid entries = " << valid.size()
            << ", unique pitch = " << pitches.size()
            << ", unique Vcath = " << vcats.size() << "\n";

  std::cerr << "[TH2 Ex2 map] pitch range = "
            << pitches.front() << " to " << pitches.back() << " mm\n";
  std::cerr << "[TH2 Ex2 map] Vcath range = "
            << vcats.front() << " to " << vcats.back() << " V\n";

  const auto xEdges = MakeBinEdgesFromCenters(pitches);
  const auto yEdges = MakeBinEdgesFromCenters(vcats);

  auto h = std::make_unique<TH2D>(
      "hEx2_pitchA_vCath_TH2",
      "<E_{x}^{2}> map;A-A pitch [mm];Cathode voltage V_{c} [V];<E_{x}^{2}> [(V/cm)^{2}]",
      static_cast<int>(pitches.size()), xEdges.data(),
      static_cast<int>(vcats.size()),   yEdges.data());

  h->SetStats(0);

  for (const auto& e : valid) {
    const int ix0 = FindIndexWithTol(pitches, e.pitchA_mm, 1.0e-5);
    const int iy0 = FindIndexWithTol(vcats,   e.vCat,      1.0e-5);

    if (ix0 < 0 || iy0 < 0) continue;

    h->SetBinContent(ix0 + 1, iy0 + 1, e.ex2_mean);
  }

  auto c = std::make_unique<TCanvas>(
      "cEx2_pitchA_vCath_TH2",
      "Ex2 pitchA-vCath TH2",
      1100, 850);
  ...
  const fs::path pngPath  = outDir / "Ex2_pitchA_vCath_TH2_colz.png";
  const fs::path pdfPath  = outDir / "Ex2_pitchA_vCath_TH2_colz.pdf";
  const fs::path rootPath = outDir / "Ex2_pitchA_vCath_TH2_colz.root";
  ...
  {
    TFile fout(rootPath.string().c_str(), "RECREATE");
    h->Write("hEx2_pitchA_vCath_TH2");
    c->Write("cEx2_pitchA_vCath_TH2");
    fout.Close();
  }

  std::cout << "[Info] Saved TH2 Ex2 heatmap: " << pngPath << "\n";
}

static void SaveEx2PitchVcat3DPlot(const fs::path& outDir,
                                   const std::vector<ConfigRankingEntry>& entries,
                                   const int topN = 20) {
  fs::create_directories(outDir);
  ...
  std::sort(valid.begin(), valid.end(), [](const auto& a, const auto& b) { return a.ex2_mean < b.ex2_mean; });

  std::vector<double> xPitchMm, yVcat, zEx2;
  for (const auto& e : valid) {
    xPitchMm.push_back(e.pitchA_mm);
    yVcat.push_back(e.vCat);
    zEx2.push_back(e.ex2_mean);
  }

  {
    ...
    std::set<double> uniquePitch;
    std::set<double> uniqueVcat;

    for (size_t i = 0; i < xPitchMm.size(); ++i) {
      xmin = std::min(xmin, xPitchMm[i]);
      xmax = std::max(xmax, xPitchMm[i]);
      ymin = std::min(ymin, yVcat[i]);
      ymax = std::max(ymax, yVcat[i]);
      uniquePitch.insert(std::round(xPitchMm[i] * 1.0e6) / 1.0e6);
      uniqueVcat.insert(std::round(yVcat[i] * 1.0e6) / 1.0e6);
    }

    std::cerr << "[DEBUG Ex2 TGraph2D] N points = " << xPitchMm.size() << "\n";
    std::cerr << "[DEBUG Ex2 TGraph2D] pitch range = "
              << xmin << " to " << xmax << " mm, unique = "
              << uniquePitch.size() << "\n";
    std::cerr << "[DEBUG Ex2 TGraph2D] Vcath range = "
              << ymin << " to " << ymax << " V, unique = "
              << uniqueVcat.size() << "\n";
  }

  auto g = std::make_unique<TGraph2D>((int)valid.size(), xPitchMm.data(), yVcat.data(), zEx2.data());
  g->SetName("gEx2_pitchA_vCath");
  g->SetTitle("<E_{x}^{2}> surface;A-A pitch [mm];Cathode voltage V_{c} [V];<E_{x}^{2}> [(V/cm)^{2}]");
  g->SetNpx(80);
  g->SetNpy(80);

  auto c = std::make_unique<TCanvas>("cEx2PitchVcat3D", "Ex2: pitchA vs Vcath", 1100, 850);
  c->SetRightMargin(0.16);
  g->Draw("SURF1");
  gPad->SetTheta(35.0);
  gPad->SetPhi(35.0);
  c->SaveAs((outDir / "Ex2_pitchA_vCath_surface3D.png").string().c_str());
  c->SaveAs((outDir / "Ex2_pitchA_vCath_surface3D.pdf").string().c_str());

  auto cColz = std::make_unique<TCanvas>("cEx2PitchVcatCOLZ", "Ex2: pitchA vs Vcath COLZ", 1100, 850);
  cColz->SetRightMargin(0.16);
  g->Draw("COLZ");
  cColz->SaveAs((outDir / "Ex2_pitchA_vCath_colz.png").string().c_str());
  cColz->SaveAs((outDir / "Ex2_pitchA_vCath_colz.pdf").string().c_str());

  {
    TFile fout((outDir / "Ex2_pitchA_vCath_3D.root").string().c_str(), "RECREATE");
    g->Write("gEx2_pitchA_vCath");
    c->Write("cEx2_pitchA_vCath_surface3D");
    cColz->Write("cEx2_pitchA_vCath_colz");

    TTree t("scan", "Vcath and anode-pitch scan for Ex2");
    double pitchA_mm = 0.0, pitchA_cm = 0.0, vCath = 0.0, ex2_mean = 0.0, ex_abs_max = 0.0;
    double rms_cm = 0.0, r2 = 0.0, x_eval_cm = 0.0;
    char config_name[256];
    t.Branch("pitchA_mm", &pitchA_mm);
    t.Branch("pitchA_cm", &pitchA_cm);
    t.Branch("Vcath_V", &vCath);
    t.Branch("Ex2_mean", &ex2_mean);
    t.Branch("Ex_abs_max", &ex_abs_max);
    t.Branch("Lt_RMS_cm", &rms_cm);
    t.Branch("Lt_R2", &r2);
    t.Branch("x_eval_cm", &x_eval_cm);
    t.Branch("config_name", config_name, "config_name/C");
    for (const auto& e : valid) {
      pitchA_mm = e.pitchA_mm;
      pitchA_cm = e.pitchA_cm;
  ...
  {
    std::ofstream fout(outDir / "Ex2_pitchA_vCath_all.csv");
    if (fout) {
      fout << "rank_by_Ex2,config_name,Vcath_V,pitchA_mm,pitchA_cm,x_eval_cm,Ex2_mean,Ex_abs_max,Lt_RMS_cm,Lt_R2,Ex_profile\n";
      for (size_t i = 0; i < valid.size(); ++i) {
        const auto& e = valid[i];
        fout << (i + 1) << "," << e.conf_name << "," << e.vCat << "," << e.pitchA_mm << ","
             << e.pitchA_cm << "," << e.ex_boundary_x_cm << "," << e.ex2_mean << ","
             << e.ex_abs_max << "," << e.residual_rms << "," << e.r2 << "," << e.ex_profile_png << "\n";
      }
    }
  }

  {
    std::ofstream fout(outDir / "Ex2_pitchA_vCath_ranking.md");
    if (fout) {
      fout << "# Raw <Ex^2> ranking: Vcath / A-A pitch sweep\n\n";
      fout << "- Smaller raw <Ex^2> is better.\n";
      fout << "- ROOT file: `Ex2_pitchA_vCath_3D.root` contains `gEx2_pitchA_vCath` and `scan`.\n\n";
      fout << "| Rank | Config | Vcath [V] | A-A pitch [mm] | x_eval [cm] | <Ex^2> | max|Ex| | Lt RMS [cm] | R^2 | Ex profile |\n";
      fout << "|---:|:---|---:|---:|---:|---:|---:|---:|---:|:---|\n";
      for (int i = 0; i < std::min<int>(topN, (int)valid.size()); ++i) {
        const auto& e = valid[i];
        fout << "| " << (i + 1)
             << " | `" << e.conf_name << "`"
             << " | " << e.vCat
...
int main(int argc, char** argv) {
  ...
  const char* timeHistFile =
      (argc > 1) ? argv[1] : "./root/run_20260322_203004/analysis_L0_prim_exact/t_hist_nt.csv";

  std::vector<double> vCat_list = {
      -500, -600, -700, -800, -900, -1000,
      -1100, -1200, -1300, -1400, -1500, -1600, -1700
  };
  std::vector<double> vAnode_list = {0.0};
  // A-only: potential wire is not included, so Vp is not scanned.
  std::vector<double> vPW_list = {0.0};

  // A-A pitch scan [cm]. 0.20 cm = 2.0 mm.
  std::vector<double> pitchA_list = {
      0.12, 0.14, 0.16, 0.18, 0.20,
      0.22, 0.24, 0.26, 0.28, 0.30
  };

  std::vector<double> dAnode_list = {0.0020};  // [cm] = 20 um diameter
  std::vector<double> dPW_list    = {0.0};     // A-only: unused
  ...
  fs::path outBaseDir = fs::path("opt_20260426_a");
  ...
  fs::path exProfileSummaryDir = sweepBaseDir / "ExProfile_midcell_Summary";
  ...
  for (double dPW : dPW_list) {
    ...
    char conf_name[160];
    TString textVolt = TString::Format("Voltages: Vc=%.0fV, Va=%.0fV", vCat, vAnode);
    TString textDiam;
    TString textPitch = TString::Format("Pitch: A-A = %.2f mm", pitchA * 10.0);
    TString textGeom;

#if defined(GEOM_PLATE)
    std::snprintf(conf_name, sizeof(conf_name),
                  "vC_%.0f_vA_%.0f_pA_%.0fum_dA_%.0f",
                  vCat, vAnode, pitchA * 1e4, dAnode * 1e4);
    textDiam = TString::Format("Diameters: da=%.0fum", dAnode * 1e4);
    textGeom = TString::Format("Geom: plane cathode, gap = %.2f mm", 0.50 * 10.0);
#elif defined(GEOM_WIRE)
    std::snprintf(conf_name, sizeof(conf_name),
                  "vC_%.0f_vA_%.0f_pA_%.0fum_dC_%.0f_dA_%.0f",
                  vCat, vAnode, pitchA * 1e4, dCat * 1e4, dAnode * 1e4);
    textDiam = TString::Format("Diameters: dc=%.0fum, da=%.0fum",
                               dCat * 1e4, dAnode * 1e4);
    textGeom = TString::Format("Geom: wire cathode, cath pitch = %.2f mm, cath phase = %.2f mm, gap = %.2f mm",
                               pitchA * 10.0, 0.5 * pitchA * 10.0, 0.50 * 10.0);
#endif
    ...
    latexConfig.DrawLatex(0.15, 0.77, textPitch);
    latexConfig.DrawLatex(0.15, 0.73, textGeom);
    ...
    // =========================================================
    const double xBoundaryEval = 0.5 * sensePitchX;
    ...
    latexEx.DrawLatex(0.15, 0.80, textPitch);
    latexEx.DrawLatex(0.15, 0.76, textGeom);
    latexEx.DrawLatex(0.15, 0.72, Form("x = %.3f cm = A-A midpoint", xBoundaryEval));
    latexEx.DrawLatex(0.15, 0.68, Form("<Ex^{2}> = %.6e", exSumm.ex2_mean));
    latexEx.DrawLatex(0.15, 0.64, Form("max|Ex| = %.6e", exSumm.ex_abs_max));
...

    {
      ...
      p.vPW = vPW;
      p.vCat = vCat;
      p.pitchA_cm = sensePitchX;
      p.pitchA_mm = sensePitchX * 10.0;
    ...
    cCombo->cd();
    latexEval.DrawLatex(0.15, 0.65, textRms);
    latexEval.DrawLatex(0.15, 0.61, textR2);
    latexEval.DrawLatex(0.15, 0.57, textEff);
    if (fieldSumm.valid) latexEval.DrawLatex(0.15, 0.53, textField);
    cCombo->SaveAs(combo_png.c_str());
    cCombo->SaveAs((emapsSummaryDir / Form("fieldmap_combo_%s.png", conf_name)).string().c_str());
...

    {
      ...
      e.vAnode = vAnode;
      e.vPW = vPW;
      e.pitchA_cm = sensePitchX;
      e.pitchA_mm = sensePitchX * 10.0;
      e.dCat_um = dCat * 1e4;
  ...
  ComputeAOnlyScores(rankingEntries);
  SaveExOverlayPlots(exProfileSummaryDir, exProfiles, "Ex vs Y at A-A midpoint");
  SaveAllWireLtSummaryPlots(allWireLtSummaryDir, rankingEntries, 10);

  fs::path pitchVcatEx2Dir = totalScoreDir / "PitchVcat_Ex2";
  fs::create_directories(pitchVcatEx2Dir);

  WriteAOnlyVcatPitchRankingMarkdown(
      pitchVcatEx2Dir / "Aonly_Vcath_pitch_ranking.md",
      rankingEntries,
      20
  );

  WriteAOnlyVcatPitchScoreCsv(
      pitchVcatEx2Dir / "Aonly_Vcath_pitch_scores.csv",
      rankingEntries
  );

  for (double pitchMm : {1.20, 1.40, 1.60, 1.80, 2.00, 2.20, 2.40, 2.60, 2.80, 3.00}) {
    SaveEx2VsVcatForFixedPitchPlot(pitchVcatEx2Dir, rankingEntries, pitchMm);
  }

  for (double vCatFixed : vCat_list) {
    SaveEx2VsPitchForFixedVcatPlot(pitchVcatEx2Dir, rankingEntries, vCatFixed);
  }

  SaveEx2PitchVcatHeatmapTH2(pitchVcatEx2Dir, rankingEntries);

  SaveEx2PitchVcat3DPlot(pitchVcatEx2Dir, rankingEntries, 20);
  ...
  std::printf("[Info] Vcath-pitch Ex2 dir: %s\n", (totalScoreDir / "PitchVcat_Ex2").string().c_str());
  std::printf("[Info] A-only ranking saved to: %s\n", (totalScoreDir / "PitchVcat_Ex2" / "Aonly_Vcath_pitch_ranking.md").string().c_str());
  std::printf("[Info] Vcath-pitch Ex2 3D ROOT: %s\n", (totalScoreDir / "PitchVcat_Ex2" / "Ex2_pitchA_vCath_3D.root").string().c_str());
  return 0;
}
