...
const double vCat  = -1500.0;   // [V]
  ...
  const double vPW        = vCat * (55.0 / 700.0);   // = -55 V when vCat=-700 V
  ...
  // Wire cathode 用
  const double pitchSense = 0.20;      // [cm] A-A
  const double pwOffset   = 0.0;       // [cm]
  const double pitchCath  = 0.20;      // [cm] cathode wire pitch
  const double cathPhase  = 0.1;       // [cm]
  const double rAn        = 1.0e-3;    // [cm]
  const double rPW        = 2.5e-3;    // [cm]
  const double rCat       = 2.5e-3;    // [cm]
  const double vPW        = vCat * (50.0 / 700.0);   // = -50 V when vCat=-700 V
