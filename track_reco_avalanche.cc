...
// DrawElectrodes: スタック確保 + DrawClone() でメモリリーク修正済み (問題 No.4')
// (旧: new TLine/TEllipse → Draw() はポインタを失いパッド依存の暗默オーナーシップに頼っていた)
static void DrawElectrodes(const Detector::Geometry& g) {
  ...
  for (const auto& e : g.electrodes) {
    if (e.kind == Detector::ElectrodeKind::PlaneY) {
      TLine ln(xmin, e.y, xmax, e.y);
      ln.SetLineColor(kGray + 3); ln.SetLineStyle(2); ln.SetLineWidth(3);
      ln.DrawClone("same");  // パッドがコピーのオーナーになる
      continue;
    }
    ...
    // 電極円を描く共通ラムダ (スタック確保 + DrawClone)
    auto drawCircle = [&](double x) {
      TEllipse cir(x, e.y, e.radius, e.radius);
      ...
      cir.SetLineColor(kBlack); cir.SetLineWidth(1);
      cir.DrawClone("same");  // パッドがコピーのオーナーになる
    };
...
int main(int argc, char** argv) {
...
#elif defined(GEOM_WIRE)
  geo = Detector::PAP_WireCathode_Periodic(0.60, 0.50);
#endif
  ...
  for (const auto& e : geo.electrodes) {
      auto lower = [](std::string s){ std::transform(s.begin(), s.end(), s.begin(), ::tolower); return s; };
      if (lower(e.name).find("cathode") != std::string::npos) { vCat_val = e.V; break; }
  }
  ...
  double t_x_true, t_x_reco, t_diff, t_abs_diff, t_a_true, t_b_true, t_a_reco, t_b_reco, t_chi2; int t_event_id;
  ...
  tree->Branch("x_reco", &t_x_reco); tree->Branch("diff_x", &t_diff); tree->Branch("abs_diff", &t_abs_diff); tree->Branch("chi2",&t_chi2);
  ...
  TimeHist th; LoadTimeHistogram(timeHistFile, th);
  ...
  TFile* fDriftOut = new TFile(driftRootOutPath.c_str(), "RECREATE"); fDriftOut->cd();
  TTree* tDrift = new TTree("driftTree", "Electron Drift Info"); tDrift->SetDirectory(fDriftOut); 
  ...
  TFile* fGainOut = new TFile(gainRootOutPath.c_str(), "RECREATE"); fGainOut->cd();
  TTree* tGain = new TTree("gainTree", "Avalanche Gain Info"); tGain->SetDirectory(fGainOut);
  ...
  // --- 検算用ツリー (Weighting Potential & Raw Charge) ---
  std::string verifyRootOutPath = (trackDir / Form("verify_info_job%d.root", job_seed)).string();
  TFile* fVerifyOut = new TFile(verifyRootOutPath.c_str(), "RECREATE"); fVerifyOut->cd();
  TTree* tVerify = new TTree("verifyTree", "Shockley-Ramo Verification"); tVerify->SetDirectory(fVerifyOut);
...
// --- 2.1 アノードの1Dプロファイル ---
  ...
  for (int ev = job_seed; ev < job_seed + 1; ++ev) {
        ...
        const double true_angle_deg = -33.0; 
        ...
        for (auto& [wName, data] : globalGainMap) {
            ...
            fGainOut->cd(); tGain->Fill(); evGainData.push_back(data);
        }
        ...
        latex.SetTextColor(kBlack); latex.DrawLatex(0.15, 0.65, Form("Diff: |x_true - x_reco| = %.4f cm", abs_diff_val));
        ...
        t_diff = x_intercept_true - x_intercept_reco; t_abs_diff = abs_diff_val; t_chi2 = min_chi2;
        tree->Fill();
    } 
